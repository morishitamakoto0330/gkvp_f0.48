...
!-------------------------------------------------------------------------------
  ...
  use GKV_advnc,  only: caldlt_LB, caldlt_full
...
!--------------------------------------
  SUBROUTINE set_init( ff, phi, Al, hh, dh, cft, cff, ef, time )
!--------------------------------------
    ...
    real(kind=DP), intent(out) :: time
      ...
      call set_value( ff, phi, Al, hh, dh, cft, cff, ef, time )
...
!--------------------------------------
    ...
    namelist /cmemo/ memo
    namelist /calct/ calc_type, z_bound, z_filt, z_calc, art_diff, num_triad_diag
    ...
    character(256)   :: env_string       !fj
      ...
      if ( zsp_rank == nprocz/2 .AND. rankw == 0 ) then
        open( ocol, file=trim(f_hst)//"col."//srank//"."//cnew )
      end if

      if ( zsp_rank == nprocz/2 .AND. rankw == 0 .AND. ranks == 0 ) then
        open( ocsv, file=trim(f_hst)//"csv."//cnew )
      end if
...
!--------------------------------------
    ...
    namelist /deltt/ dt_max, adapt_dt, courant_num
...
!        write( olog, * ) ""
        ...
        call vmecbzx_boozx_read( nss, ntheta, nzeta )
         ...
         isw = 0
         iz = 0
         call vmecbzx_boozx_coeff( isw,  nss,  ntheta,  nzeta,  s_input,  0._DP,  lz_l,   &  ! input 
...
!  this is new vmec-BoozXform interface  by M. Nakata & M. Nunami  (Aug. 2016)
            ...
            call vmecbzx_boozx_coeff( isw,  nss,  ntheta,  nzeta,  s_input,  zz(iz),  lz_l,   &  ! input 
                                       s_0,       q_0,     s_hat,    eps_r, phi_ax,          &  ! output
                                   omg(iz), rootg(iz),    domgdx,   domgdz, domgdy,          &
                                   gg(1,1),   gg(1,2),   gg(1,3),  gg(2,2),                  &
                                   gg(2,3),   gg(3,3)  )
...
!--------------------------------------
  SUBROUTINE set_value( ff, phi, Al, hh, dh, cft, cff, ef, time )
!--------------------------------------
