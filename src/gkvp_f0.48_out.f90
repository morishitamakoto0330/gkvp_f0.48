...
!-------------------------------------------------------------------------------
  ...
  use GKV_intgrl, only: intgrl_thet, intgrl_fsrf, &
                        intgrl_v0_moment, intgrl_v1_moment, intgrl_v2_moment
...
!--------------------------------------
  SUBROUTINE out_cntrl( ff, phi, Al, dh, cft, cff, time, id )
!--------------------------------------
    ...
    complex(kind=DP), intent(in), &
      dimension(-nx:nx,0:ny,-nz:nz-1,1:2*nv,0:nm) :: dh, cft, cff
    ...
    real(kind=DP), save :: tout_fxv, tout_ptn, tout_eng
      ...
      if( id == 0 ) then
        ...
        if ( time == 0._DP ) then
          call wrt ( ff, phi, Al, dh, cft, cff, time, 0 )
          call wrt ( ff, phi, Al, dh, cft, cff, time, 1 )
          call wrt ( ff, phi, Al, dh, cft, cff, time, 2 )
        end if
...
!--------------------------------------
  SUBROUTINE wrt ( ff, phi, Al, dh, cft, cff, time, id )
!--------------------------------------
    ...
    complex(kind=DP), dimension(-nx:nx,0:ny,1:2*nv,0:nm) :: fout
...
!$OMP parallel do collapse(2)
        ...
        call balance ( ff, phi, Al, dh, cft, cff, time,                    &
...
!--------------------------------------
  SUBROUTINE balance ( ff, phi, Al, dh, cft, cff, time,                    &
                       entrpy, fenegy, menegy, peint, pmint, neint, nmint, &
                       dcd, pflux_es, pflux_em, eflux_es, eflux_em )
!--------------------------------------
!     Check the entropy balance equation
    ...
    complex(kind=DP), dimension(:,:,:,:,:), allocatable :: wf
    complex(kind=DP), dimension(-nx:nx,0:ny,-nz:nz-1)   :: dens, upara, pres, qpara
    complex(kind=DP), dimension(-nx:nx,0:ny,-nz:nz-1)   :: ni, wc3
      ...
      do im = 0, nm
        do iv = 1, 2*nv
          do iz = -nz, nz-1
            do my = ist_y, iend_y
              do mx = -nx, nx
                wf(mx,my,iz,iv,im)   = fcs(ranks) / Znum(ranks) * (cft(mx,my,iz,iv,im)+cff(mx,my,iz,iv,im))  &
...
! --- divergence trap
          if ( entrpy_nz > 1.d100 ) then 
            if (rank == 0) write(olog,*) "DIVERGE!! at ", time, entrpy_nz
            stop "STOPPED!"
          end if
! --- 
...
!     Calculate turbulent flux
    ...
    real(kind=DP),              &
      dimension(-nx:nx,0:ny,-nz:nz-1)        :: wr3
    integer  ::  mx, my, iz
...
!--------------------------------------
    ...
    complex(kind=DP), dimension(-nx:nx,0:ny,-nz:nz-1,1:2*nv,0:nm) :: wf
    complex(kind=DP), &
