...
!      from the VMEC and BoozXform using using an external(like IGS) code.  
!                                 
!            by M. Nakata & M. Nunami
!                                                                  ( Aug 2016 )
!
!-------------------------------------------------------------------------------
...
!--------------------------------------
    ...
    allocate(dBds_bz(1:nss,0:ntheta,0:nzeta),dBdt_bz(1:nss,0:ntheta,0:nzeta),dBdz_bz(1:nss,0:ntheta,0:nzeta))
      ...
      open( ibzx, file=trim(f_bozx)//"metric_boozer.bin.dat", status="old", action="read", form="unformatted", CONVERT='LITTLE_ENDIAN' )
...
!----------------------------------------------------------------------------------
  SUBROUTINE vmecbzx_boozx_coeff( isw,  nss,  ntheta,  nzeta,  s_input,  zz,  lz_l,        &  ! input 
...
!----------------------------------------------------------------------------------
    ...
    integer, intent(in)        :: isw, nss, ntheta, nzeta
...
! --- local variables 
    ...
    if ( isw == 0 ) then 
      ...
      nz0 = pi*nz/lz_l        ! get grid number on z=[0,pi)
