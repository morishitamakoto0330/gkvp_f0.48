...
!-------------------------------------------------------------------------------
  ...
  integer, parameter :: nhead = 4  ! Sizes of header and footer in sequential binary files


!%%% DIAG parameters %%%
  integer, parameter :: snum = 1      ! begining of simulation runs
  integer, parameter :: enum = 3      ! end of simulation runs
  ...
  logical, parameter :: flag_mominkxky = .true.
  logical, parameter :: flag_mominxy   = .true.
  logical, parameter :: flag_mominz    = .true.
  logical, parameter :: flag_mominxyz  = .false.
...
!%%%%%%%%%%%%%%%%%%%%%%
  ...
  real(kind=DP), dimension(-nx:nx,0:global_ny,-global_nz:global_nz-1) :: gksq, fct_e_energy, fct_m_energy
  ...
  integer, parameter :: recl_cnt = 2*nhead+DP+(2*nx+1)*(ny+1)*(2*nz)*(2*nv)*(nm+1)*(2*DP), &
                        recl_phi = 2*nhead+DP+(2*nx+1)*(ny+1)*(2*nz)*(2*DP),   &
                        recl_Al  = 2*nhead+DP+(2*nx+1)*(ny+1)*(2*nz)*(2*DP),   &
                        recl_mom = 2*nhead+DP+(2*nx+1)*(ny+1)*(2*nz)*6*(2*DP), &
                        recl_trn = 2*nhead+DP+(2*nx+1)*(ny+1)*4*DP
  ...
  integer :: loop_sta, loop_end
    ...
    if ( flag_cnt ) then
      ...
      do inum = 1, enum
       ...
       !%%% for GNU fortran %%%
        call stat( "./cnt/gkvp_f0.30.000000.cnt."//cnum, statb )
        filesize = statb(8)
       !%%% for Intel fortran %%%
       ! inquire( file="./cnt/gkvp_f0.30.000000.cnt."//cnum, size=filesize )
        loop_end = loop + filesize/recl_cnt - 1
        write( cloop, fmt="(i8.8)" ) loop_end
        write( olog, * ) " ### loop_end = "//cloop

        if ( inum < snum ) then
          write( olog, * ) " # Skip diag_cnt."
        else
          write( olog, * ) " # Begin diag_cnt."
          call diag_cnt( inum, loop )
          write( olog, * ) " # End diag_cnt."
        end if
    ...
    if ( flag_mom ) then
      ...
      do inum = 1, enum
        ...
        write( olog, * ) " ### loop_sta = "//cloop
       !%%% for GNU fortran %%%
        call stat( "./phi/gkvp_f0.30.000000.0.phi."//cnum, statb )
        filesize = statb(8)
       !%%% for Intel fortran %%%
       ! inquire( file="./phi/gkvp_f0.30.000000.0.phi."//cnum, size=filesize )
        loop_end = loop + filesize/recl_phi - 1
        write( cloop, fmt="(i8.8)" ) loop_end
        write( olog, * ) " ### loop_end = "//cloop

        if ( inum < snum ) then
          write( olog, * ) " # Skip diag_mom."
        else
          write( olog, * ) " # Begin diag_mom."
          call diag_mom( inum, loop )
          write( olog, * ) " # End diag_mom."
        end if
    ...
    if ( flag_trn ) then
      ...
      do inum = 1, enum
        ...
        write( olog, * ) " ### loop_sta = "//cloop
       !%%% for GNU fortran %%%
        call stat( "./phi/gkvp_f0.30.000000.0.trn."//cnum, statb )
        filesize = statb(8)
       !%%% for Intel fortran %%%
       ! inquire( file="./phi/gkvp_f0.30.000000.0.trn."//cnum, size=filesize )
        loop_end = loop + filesize/recl_trn - 1
        write( cloop, fmt="(i8.8)" ) loop_end
        write( olog, * ) " ### loop_end = "//cloop

        if ( inum < snum ) then
          write( olog, * ) " # Skip diag_trn."
        else
          write( olog, * ) " # Begin diag_trn."
          call diag_trn( inum, loop )
          write( olog, * ) " # End diag_trn."
        end if
    ...
    implicit none
      ...
      if ( flag_cnt ) then
        ...
        open( pffinzv, file="./plot_ffinzv.gn" )
          ...
          write( pffinzv, * ) "# set terminal png enhanced"
      ...
      if ( flag_mom ) then
        ...
        if ( flag_mominkxky ) then
          open( pmominkxky, file="./plot_mominkxky.gn" )
            ...
            write( pmominkxky, * ) "# set terminal png enhanced"
...
!$OMP parallel do private( bb, gg0 )
      do iz = -global_nz, -global_nz-1
        do my = 0, global_ny
          do mx = -nx, nx
            fct_e_energy(mx,my,iz) = lambda_i * gksq(mx,my,iz)
            do is = 0, ns-1
              bb   = gksq(mx,my,iz) / gomg(iz)**2 &
...
!$OMP parallel do
      ...
      write( olog, * ) "flag_mominxyz    =", flag_mominxyz
      write( olog, * ) "flag_trn         =", flag_trn
      ...
      do
        ...
        if ( loop >= 0 .and. mod(loop,loopskip)==0 ) then  !%%% loop control
          ...
          !  read( unit=100000+ir, iostat=ios ) time, ff
            read( unit=100000+ir, iostat=ios, rec=row ) head, time, ff, foot
            if ( ios /= 0 ) exit
!$OMP parallel do private(mx,my,iz,iv,im,gmy,giz,giv,gim)
          ...
          if ( ios /= 0 ) then
            write( olog, * ) " # End of file is detected. iostat, rec, unit-ID = ",  &
                             ios, row, 100000+ir
            exit
          end if
          ...
          if ( flag_ffinzv ) then
            ...
            my = 4
            im = 4
            call ffinzv( mx, my, im, inum, loop, time, gff )
          end if
...
!     Write |ff| in zz, vl space
  ...
  ! --- local variables
    ...
    integer :: iz, iv
      ...
      open( offinzv, file="./plt/ffinzv_mx"//cmx//"my"//cmy//"im"//cim//"_t"//cloop//".dat" )
      ...
      write( pffinzv, * ) "# set output './plt/ffinzv_mx"//cmx//"my"//cmy//"im"//cim//"_t"//cloop//".png'"
      write( pffinzv, * ) "  set title 'time =",time,"'"
      write( pffinzv, * ) "  splot './plt/ffinzv_mx"//cmx//"my"//cmy//"im"//cim//"_t"//cloop//".dat' u 1:2:3 w l"
      write( pffinzv, * ) "  pause 0.2"
        ...
        if ( loop >= 0 .and. mod(loop,loopskip)==0 ) then  !%%% loop control
          ...
          do ir = 0, nproc - 1
            ...
            !  read( unit=300000+ir, iostat=ios ) time, Al
              read( unit=200000+ir, iostat=ios, rec=row ) head, time, phi, foot
              read( unit=300000+ir, iostat=ios, rec=row ) head, time, Al, foot
              if ( ios /= 0 ) exit
!$OMP parallel do private(mx,my,iz,gmy,giz)
            ...
            !  read( unit=400000+ir, iostat=ios ) time, dens, upara, ppara, pperp, qlpara, qlperp
              read( unit=400000+ir, iostat=ios, rec=row ) head, time, dens, upara, ppara, pperp, qlpara, qlperp, foot
              if ( ios /= 0 ) exit
!$OMP parallel do private(mx,my,iz,gmy,giz)
              do iz = -nz, nz-1
                giz = - global_nz + 2*nz * rankz + iz + nz
          ...
          if ( ios /= 0 ) then
            write( olog, * ) " # End of file is detected. iostat, rec, unit-ID = ",  &
                           ios, row, 200000+ir, 300000+ir, 400000+ir
            exit
          end if
          ...
          if ( flag_mominxyz ) then
            call mominxyz( inum, loop, time,  &
                           gphi, gAl, gdens, gupara, gppara, gpperp, gqlpara, gqlperp )
          end if
...
!     Write phi in kx, ky space
  ...
  ! --- local variables
    ...
    real(kind=DP), dimension(-nx:nx,0:global_ny)           :: phi2, Al2, fenegy2, menegy2
!    real(kind=DP), dimension(-nx:nx,0:global_ny,0:ns-1)    :: dens2, upara2, pres2, qpara2,  &
!                                                              phidens, uparaAl, phipres, qparaAl
    real(kind=DP), dimension(-nx:nx,0:global_ny,0:ns-1)    :: dens2, upara2, ppara2, pperp2, qlpara2, qlperp2,  &
                                                              phidens, uparaAl, phippara, phipperp, qlparaAl, qlperpAl
    real(kind=DP),  &
...
!$OMP parallel do
        do iz = -global_nz, global_nz-1
          do my = 0, global_ny
            do mx = -nx, nx
              wr3(mx,my,iz) = real( gppara(mx,my,iz,is) * conjg( gppara(mx,my,iz,is) ), kind=DP )
            end do
          end do
        end do
        call thet_ave_r ( wr3, ppara2(:,:,is) )
!$OMP parallel do
        do iz = -global_nz, global_nz-1
          do my = 0, global_ny
            do mx = -nx, nx
              wr3(mx,my,iz) = real( gpperp(mx,my,iz,is) * conjg( gpperp(mx,my,iz,is) ), kind=DP )
            end do
          end do
        end do
        call thet_ave_r ( wr3, pperp2(:,:,is) )
!$OMP parallel do
        do iz = -global_nz, global_nz-1
          do my = 0, global_ny
            do mx = -nx, nx
              wr3(mx,my,iz) = real( gqlpara(mx,my,iz,is) * conjg( gqlpara(mx,my,iz,is) ), kind=DP )
            end do
          end do
        end do
        call thet_ave_r ( wr3, qlpara2(:,:,is) )
!$OMP parallel do
        do iz = -global_nz, global_nz-1
          do my = 0, global_ny
            do mx = -nx, nx
              wr3(mx,my,iz) = real( gqlperp(mx,my,iz,is) * conjg( gqlperp(mx,my,iz,is) ), kind=DP )
            end do
          end do
        end do
        call thet_ave_r ( wr3, qlperp2(:,:,is) )
!$OMP parallel do
        do iz = -global_nz, global_nz-1
          do my = 0, global_ny
            do mx = -nx, nx
...
!$OMP parallel do
        do iz = -global_nz, global_nz-1
          do my = 0, global_ny
            do mx = -nx, nx
              wr3(mx,my,iz) = aimag( gphi(mx,my,iz) * conjg( gppara(mx,my,iz,is) ) )
            end do
          end do
        end do
        call thet_ave_r ( wr3, phippara(:,:,is) )
!$OMP parallel do
        do iz = -global_nz, global_nz-1
          do my = 0, global_ny
            do mx = -nx, nx
              wr3(mx,my,iz) = aimag( gphi(mx,my,iz) * conjg( gpperp(mx,my,iz,is) ) )
            end do
          end do
        end do
        call thet_ave_r ( wr3, phipperp(:,:,is) )
!$OMP parallel do
        do iz = -global_nz, global_nz-1
          do my = 0, global_ny
            do mx = -nx, nx
              wr3(mx,my,iz) = aimag( gqlpara(mx,my,iz,is) * conjg( gAl(mx,my,iz) ) )
            end do
          end do
        end do
        call thet_ave_r ( wr3, qlparaAl(:,:,is) )
!$OMP parallel do
        do iz = -global_nz, global_nz-1
          do my = 0, global_ny
            do mx = -nx, nx
              wr3(mx,my,iz) = aimag( gqlperp(mx,my,iz,is) * conjg( gAl(mx,my,iz) ) )
            end do
          end do
        end do
        call thet_ave_r ( wr3, qlperpAl(:,:,is) )
      end do

      write( cnum, fmt="(i3.3)" ) inum
      write( cloop, fmt="(i8.8)" ) loop
      open( omominkxky, file="./plt/mominkxky_t"//cloop//".dat" )
        ...
        write( omominkxky, "(99a17)" ) "#              kx","ky",            &
                                       ...
                                       "Im[<phi,ppara>]","Im[<phi,pperp>]", &
                                       "Im[<qlpara,Al>]","Im[<qlperp,Al>]"
        do my = 0, global_ny
          do mx = -nx, nx
            write( omominkxky, "(99G17.7E3)" ) kx(mx), gky(my), phi2(mx,my), Al2(mx,my),  &
                                             (   dens2(mx,my,is),  &
                                              ...
                                              phipperp(mx,my,is),  &
                                              qlparaAl(mx,my,is),  &
                                              qlperpAl(mx,my,is), is=0, ns-1 )
          end do
          write( omominkxky, * )
        end do
      close( omominkxky )
  
      write( pmominkxky, * ) "# set output './plt/mominkxky_t"//cloop//".png'"
      write( pmominkxky, * ) "  set title 'time =",time,"'"
      write( pmominkxky, * ) "  splot './plt/mominkxky_t"//cloop//".dat' u 1:2:3"
      ...
      do iz = -global_nz, global_nz-1
        do my = 0, global_ny
          do mx = -nx, nx
            wr3(mx,my,iz) = fct_e_energy(mx,my,iz)  &
                          * 0.5_DP * real( gphi(mx,my,iz) * conjg( gphi(mx,my,iz) ), kind=DP )
          end do
        end do
      end do
      call thet_ave_r ( wr3, fenegy2 )
      do iz = -global_nz, global_nz-1
        do my = 0, global_ny
          do mx = -nx, nx
            wr3(mx,my,iz) = fct_m_energy(mx,my,iz)  &
                          * 0.5_DP * real( gAl(mx,my,iz) * conjg( gAl(mx,my,iz) ), kind=DP )
          end do
        end do
      end do
      call thet_ave_r ( wr3, menegy2 )

      open( omominkxky, file="./plt/fldinkxky_t"//cloop//".dat" )
        write( omominkxky, "(a17i17a17G17.7E3)" )  &
                                       "#           loop=",loop, "time=",time
        write( omominkxky, "(99a17)" ) "#              kx","ky","W_E","W_M"
        do my = 0, global_ny
          do mx = -nx, nx
            write( omominkxky, "(99G17.7E3)" ) kx(mx), gky(my), fenegy2(mx,my), menegy2(mx,my)
          end do
          write( omominkxky, * )
        end do
      close( omominkxky )
...
!$OMP parallel do
      ...
      write( cnum, fmt="(i3.3)" ) inum
      write( cloop, fmt="(i8.8)" ) loop
      open( omominxy, file="./plt/mominxy_z"//ciz//"_t"//cloop//".dat" )
...
!     Write phi in zz space
  ...
  ! --- local variables
    ...
    integer :: iz, is
      ...
      write( cnum, fmt="(i3.3)" ) inum
      write( cloop, fmt="(i8.8)" ) loop
      open( omominz, file="./plt/mominz_mx"//cmx//"my"//cmy//"_t"//cloop//".dat" )
...
!--------------------------------------
  SUBROUTINE mominxyz( inum, loop, time, gphi, gAl, gdens, gupara, gppara, gpperp, gqlpara, gqlperp )
!--------------------------------------
!     Write 3D data of phi in vtk format
  ...
  ! --- local variables
    ...
    character*3 :: cnum
    character*8 :: cloop
    integer :: mx, my, iz, is, mwp

    character*8, parameter :: dataset = "points"  ! "points" or "grid"
  

      write( cnum, fmt="(i3.3)" ) inum
      write( cloop, fmt="(i8.8)" ) loop

      open( omominxyz, file="./plt/phiinxyz_t"//cloop//".vtk" )

        write( omominxyz, '(A)' ) "# vtk DataFile Version 2.0"
        write( omominxyz, '(A,G22.12E3,A,G22.12E3,A,G22.12E3)' ) "lx= ", lx, " ly= ", ly, " lz= ", lz
        write( omominxyz, '(A)' ) "ASCII"
        if ( dataset == "points" ) then
          write( omominxyz, '(A)' ) "DATASET STRUCTURED_POINTS"
          write( omominxyz, '(A,3I15)' ) "DIMENSIONS ", 2*nxw+1, 2*nyw+1, 2*global_nz+1
          write( omominxyz, '(A)' ) "ORIGIN 0.0 0.0 0.0"
          write( omominxyz, '(A)' ) "ASPECT_RATIO 1.0 1.0 1.0"
          write( omominxyz, * ) 
        else if ( dataset == "grid" ) then
          write( omominxyz, '(A)' ) "DATASET STRUCTURED_GRID"
          write( omominxyz, '(A,3I15)' ) "DIMENSIONS ", 2*nxw+1, 2*nyw+1, 2*global_nz+1
          write( omominxyz, '(A,I15,A)' ) "POINTS ", (2*nxw+1)*(2*nyw+1)*(2*global_nz+1), " float"
          ...
          do iz = -global_nz, global_nz
            ...
            do my = 0, 2*nyw
              ...
              do mx = 0, 2*nxw
                ...
                write( omominxyz, '(3G17.7E3)' ) wx_car, wy_car, wz_car
              end do
            end do
          end do
        end if
        write( omominxyz, '(A,I15)' ) "POINT_DATA ", (2*nxw+1)*(2*nyw+1)*(2*global_nz+1)
        write( omominxyz, '(3A)' ) "SCALARS ", "phi", " float 1"
        write( omominxyz, '(A)' ) "LOOKUP_TABLE default"
        do iz = -global_nz, global_nz-1
          call fft_backward_xy( gphi(:,:,iz), wr2 )
          do my = 0, 2*nyw-1
            do mx = 0, 2*nxw-1
              write( omominxyz, "(G17.7E3)" ) wr2(mx,my)
            end do
            ! mx = 2*nxw
            write( omominxyz, "(G17.7E3)" ) wr2(0,my)
          end do
          ! my = 2*nyw
            do mx = 0, 2*nxw-1
              write( omominxyz, "(G17.7E3)" ) wr2(mx,0)
            end do
            ! mx = 2*nxw
            write( omominxyz, "(G17.7E3)" ) wr2(0,0)
        end do
        ! iz = global_nz
          do my = 0, global_ny
            do mx = -nx, nx
              mwp = mx - dj(my)          ! --- mw = mx - dj for the positive-z 
                if( abs(mwp) > nx ) then
                  ...
                  wc2(mx,my) = conjg( ck(my) ) * gphi(mwp,my,-global_nz)
                end if
            end do
          end do
...
!      open( omominxyz, file="./plt/Alinxyz_t"//cloop//".vtk" )
!
!        write( omominxyz, '(A)' ) "# vtk DataFile Version 2.0"
!        write( omominxyz, '(A,G22.12E3,A,G22.12E3,A,G22.12E3)' ) "lx= ", lx, " ly= ", ly, " lz= ", lz
!        write( omominxyz, '(A)' ) "ASCII"
...
!        end if
!        write( omominxyz, '(A,I15)' ) "POINT_DATA ", (2*nxw+1)*(2*nyw+1)*(2*global_nz+1)
!        write( omominxyz, '(3A)' ) "SCALARS ", "Al", " float 1"
!        write( omominxyz, '(A)' ) "LOOKUP_TABLE default"
!        do iz = -global_nz, global_nz-1
!          call fft_backward_xy( gAl(:,:,iz), wr2 )
!          do my = 0, 2*nyw-1
!            do mx = 0, 2*nxw-1
!              write( omominxyz, "(G17.7E3)" ) wr2(mx,my)
!            end do
...
!                  wc2(mx,my) = ( 0._DP, 0._DP )
!                else
!                  wc2(mx,my) = conjg( ck(my) ) * gAl(mwp,my,-global_nz)
!                end if
!            end do
!          end do
!          call fft_backward_xy( wc2, wr2 )
...
!
!        open( omominxyz, file="./plt/densinxyz_ns"//cis//"_t"//cloop//".vtk" )
!  
!          write( omominxyz, '(A)' ) "# vtk DataFile Version 2.0"
!          write( omominxyz, '(A,G22.12E3,A,G22.12E3,A,G22.12E3)' ) "lx= ", lx, " ly= ", ly, " lz= ", lz
!          write( omominxyz, '(A)' ) "ASCII"
...
!          end if
!          write( omominxyz, '(A,I15)' ) "POINT_DATA ", (2*nxw+1)*(2*nyw+1)*(2*global_nz+1)
!          write( omominxyz, '(3A)' ) "SCALARS ", "dens", " float 1"
!          write( omominxyz, '(A)' ) "LOOKUP_TABLE default"
!          do iz = -global_nz, global_nz-1
!            call fft_backward_xy( gdens(:,:,iz,is), wr2 )
!            do my = 0, 2*nyw-1
!              do mx = 0, 2*nxw-1
!                write( omominxyz, "(G17.7E3)" ) wr2(mx,my)
!              end do
...
!                    wc2(mx,my) = ( 0._DP, 0._DP )
!                  else
!                    wc2(mx,my) = conjg( ck(my) ) * gdens(mwp,my,-global_nz,is)
!                  end if
!              end do
!            end do
!            call fft_backward_xy( wc2, wr2 )
...
!        close( omominxyz )
!
!        open( omominxyz, file="./plt/uparainxyz_ns"//cis//"_t"//cloop//".vtk" )
!  
!          write( omominxyz, '(A)' ) "# vtk DataFile Version 2.0"
!          write( omominxyz, '(A,G22.12E3,A,G22.12E3,A,G22.12E3)' ) "lx= ", lx, " ly= ", ly, " lz= ", lz
!          write( omominxyz, '(A)' ) "ASCII"
...
!          end if
!          write( omominxyz, '(A,I15)' ) "POINT_DATA ", (2*nxw+1)*(2*nyw+1)*(2*global_nz+1)
!          write( omominxyz, '(3A)' ) "SCALARS ", "upara", " float 1"
!          write( omominxyz, '(A)' ) "LOOKUP_TABLE default"
!          do iz = -global_nz, global_nz-1
!            call fft_backward_xy( gupara(:,:,iz,is), wr2 )
!            do my = 0, 2*nyw-1
!              do mx = 0, 2*nxw-1
!                write( omominxyz, "(G17.7E3)" ) wr2(mx,my)
!              end do
...
!                    wc2(mx,my) = ( 0._DP, 0._DP )
!                  else
!                    wc2(mx,my) = conjg( ck(my) ) * gupara(mwp,my,-global_nz,is)
!                  end if
!              end do
!            end do
!            call fft_backward_xy( wc2, wr2 )
...
!        close( omominxyz )
!
!        open( omominxyz, file="./plt/presinxyz_ns"//cis//"_t"//cloop//".vtk" )
!  
!          write( omominxyz, '(A)' ) "# vtk DataFile Version 2.0"
!          write( omominxyz, '(A,G22.12E3,A,G22.12E3,A,G22.12E3)' ) "lx= ", lx, " ly= ", ly, " lz= ", lz
!          write( omominxyz, '(A)' ) "ASCII"
...
!          end if
!          write( omominxyz, '(A,I15)' ) "POINT_DATA ", (2*nxw+1)*(2*nyw+1)*(2*global_nz+1)
!          write( omominxyz, '(3A)' ) "SCALARS ", "pres", " float 1"
!          write( omominxyz, '(A)' ) "LOOKUP_TABLE default"
!          do iz = -global_nz, global_nz-1
!!$OMP parallel do
!            do my = 0, global_ny
...
!                    wc2(mx,my) = ( 0._DP, 0._DP )
!                  else
!                    wc2(mx,my) = conjg( ck(my) ) * ( gppara(mwp,my,-global_nz,is) + gpperp(mwp,my,-global_nz,is) )
!                  end if
!              end do
!            end do
!            call fft_backward_xy( wc2, wr2 )
...
!        close( omominxyz )
!
!        open( omominxyz, file="./plt/qparainxyz_ns"//cis//"_t"//cloop//".vtk" )
!  
!          write( omominxyz, '(A)' ) "# vtk DataFile Version 2.0"
!          write( omominxyz, '(A,G22.12E3,A,G22.12E3,A,G22.12E3)' ) "lx= ", lx, " ly= ", ly, " lz= ", lz
!          write( omominxyz, '(A)' ) "ASCII"
...
!          end if
!          write( omominxyz, '(A,I15)' ) "POINT_DATA ", (2*nxw+1)*(2*nyw+1)*(2*global_nz+1)
!          write( omominxyz, '(3A)' ) "SCALARS ", "qpara", " float 1"
!          write( omominxyz, '(A)' ) "LOOKUP_TABLE default"
!          do iz = -global_nz, global_nz-1
!!$OMP parallel do
!            do my = 0, global_ny
...
!                    wc2(mx,my) = ( 0._DP, 0._DP )
!                  else
!                    wc2(mx,my) = conjg( ck(my) ) * ( gqlpara(mwp,my,-global_nz,is) + gqlperp(mwp,my,-global_nz,is) )
!                  end if
!              end do
!            end do
!            call fft_backward_xy( wc2, wr2 )
...
!     Data diagnostics from *.trn.*
  ...
  ! --- local variables
    ...
    real(kind=DP) :: time
    real(kind=DP),  &
      dimension(-nx:nx,0:ny)          :: entrpy, peint, pmint, neint, nmint, dcd
    real(kind=DP),  &
      dimension(-nx:nx,0:global_ny,0:ns-1)      :: gentrpy, gpeint, gpmint, gneint, gnmint, gdcd
    integer :: rankg, ranks, rank, rankw, rankz, rankv, rankm,  &
               scolor, wcolor, zcolor, vcolor, &
               spc_rank, fft_rank, zsp_rank, vel_rank
    integer :: mx, my, gmy, iz, giz
      ...
      do
        ...
        if ( loop >= 0 .and. mod(loop,loopskip)==0 ) then  !%%% loop control
          ...
          do ir = 0, nproc - 1
            ...
            if ( zsp_rank == 0 .and. vel_rank == 0 ) then
            !  read( unit=500000+ir, iostat=ios ) time, entrpy, peint, pmint, neint, nmint, dcd
              read( unit=500000+ir, iostat=ios, rec=row ) head, time, entrpy, peint, pmint, neint, nmint, dcd, foot
              if ( ios /= 0 ) exit
!$OMP parallel do private(mx,my,gmy)
                ...
                if ( gmy <= global_ny ) then
                  do mx = -nx, nx
                    gentrpy(mx,gmy,ranks) = entrpy(mx,my)
                     gpeint(mx,gmy,ranks) =  peint(mx,my)
                     gpmint(mx,gmy,ranks) =  pmint(mx,my)
          ...
          if ( flag_trninkxky ) then
            call trninkxky( inum, loop, time,  &
                              gentrpy, gpeint, gpmint, gneint, gnmint, gdcd )
          end if
...
!--------------------------------------
  SUBROUTINE trninkxky( inum, loop, time, gentrpy, gpeint, gpmint, gneint, gnmint, gdcd )
!--------------------------------------
!     Write nonlinear transfer in kx, ky
  ...
  ! --- local variables
    ...
    integer :: mx, my, is
      ...
      write( cnum, fmt="(i3.3)" ) inum
      write( cloop, fmt="(i8.8)" ) loop
      open( otrninkxky, file="./plt/trninkxky_t"//cloop//".dat" )
        write( otrninkxky, "(a17i17a17G17.7E3)" )  &
                                         "#           loop=",loop, "time=",time
        write( otrninkxky, "(99a17)" ) "#              kx","ky","entrpy","peint","pmint","neint","nmint","dcd"
        do my = 0, global_ny
          do mx = -nx, nx
