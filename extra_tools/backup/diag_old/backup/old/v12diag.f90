...
!%%% DIAG parameters %%%
  ...
  logical, parameter :: flag_transinkxky = .true.
!%%%%%%%%%%%%%%%%%%%%%%
...
!$OMP parallel do
      ...
      write( olog, * ) "flag_transinkxky =", flag_transinkxky
      write( olog, * )
...
!$OMP parallel do private(mx,my,gmy)
        ...
        if ( ios < 0 ) then
          write( olog, * ) " # All files are completed. iostat, unit-ID = ",  &
                           ios, 200000+ir, 300000+ir, 400000+ir, 500000+ir
          write( olog, * ) ""
          exit
        else if ( ios > 0 ) then
          write( olog, * ) " # I/O error is detected. iostat, unit-ID = ",  &
        ...
        if ( loop >= 0 .and. mod(loop,loopskip)==0 ) then  !%%% loop control
          ...
          if ( flag_transinkxky ) then
            call transinkxky( inum, loop, time,  &
...
!     Write nonlinear transfer in kx, ky
  ...
  ! --- local variables
    ...
    integer :: mx, my, is
      ...
      write( cnum, fmt="(i3.3)" ) inum
      write( cloop, fmt="(i8.8)" ) loop
      open( otransinkxky, file="./plt/transinkxky_t"//cloop//"."//cnum )
        write( otransinkxky, "(a17i17a17G17.7E3)" )  &
                                         "#           loop=",loop, "time=",time
        write( otransinkxky, "(99a17)" ) "#              kx","ky","entrpy","neint","nmint","dcd"
        do my = 0, global_ny
          do mx = -nx, nx
            write( otransinkxky, "(99G17.7E3)" ) kx(mx), gky(my),    &
                                               ( gentrpy(mx,my,is),  &
                                                  gneint(mx,my,is),  &
                                                  gnmint(mx,my,is),  &
