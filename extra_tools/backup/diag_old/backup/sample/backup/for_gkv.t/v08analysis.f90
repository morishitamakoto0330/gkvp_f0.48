...
!-------------------------------------------------------------------------------
  ...
  implicit none
    ...
    open( otfl, file="./plt/tfluxes.dat" )
      write( otfl, "(99a17)" ) "#            Time",                   &
                               "p_flux_es(1:ns)", "p_flux_em(1:ns)",  &
                               "h_flux_es(1:ns)", "h_flux_em(1:ns)"

    open( ptfl, file="./plot_tfluxes.gn" )
      write( ptfl, * ) "set xlabel 'Time t v_{ti}/L_n'"
      write( ptfl, * ) "set ylabel 'Particle Fluxes'"
      write( ptfl, * ) "plot './plt/tfluxes.dat' u 1:2 ti '{/Symbol G}_{iE}' w l, \"
      write( ptfl, * ) "                      '' u 1:3 ti '{/Symbol G}_{eE}' w l, \"
      write( ptfl, * ) "                      '' u 1:4 ti '{/Symbol G}_{iM}' w l, \"
      write( ptfl, * ) "                      '' u 1:5 ti '{/Symbol G}_{eM}' w l"
      write( ptfl, * ) "pause -1"
      write( ptfl, * ) "set ylabel 'Heat Fluxes'"
      write( ptfl, * ) "plot './plt/tfluxes.dat' u 1:6 ti 'Q_{iE}' w l, \"
      write( ptfl, * ) "                      '' u 1:7 ti 'Q_{eE}' w l, \"
      write( ptfl, * ) "                      '' u 1:8 ti 'Q_{iM}' w l, \"
      write( ptfl, * ) "                      '' u 1:9 ti 'Q_{eM}' w l"
      write( ptfl, * ) "pause -1"

    open( pflk, file="./plot_fluxinkxky.gn" )
      write( pflk, * ) "set pm3d map"
      write( pflk, * ) "set size square"
      write( pflk, * ) "set xlabel 'Wave number kx'"
      write( pflk, * ) "set ylabel 'Wave number ky'"
...
! --- local variables
  ...
  integer :: mx, my, iz
    ...
    write( pazz, * ) "plot './plt/Alinzz_t"//cloop//"."//cnum//"' u 3:4 w l"
...
! --- local variables
  ...
  real(kind=DP),  &
    dimension(0:nx,-ny:ny,-global_nz:global_nz-1) :: wr3
  character*3 :: cnum
  character*8 :: cloop
  integer :: mx, my, iz, is
    ...
    do is = 1, ns
      ...
      do iz = -global_nz, global_nz-1
        do my = -ny, ny
          do mx = 0, nx
            wr3(mx,my,iz) = real( &
              conjg( - ui * ky(my) * gphi(mx,my,iz) ) * gdens(mx,my,iz,is), kind=DP )
          end do
        end do
      end do
      call thet_ave_r ( wr3, p_flux_es(:,:,is) )

      do my = -ny, ny
        do mx = 1, nx
          tp_flux_es(is) = tp_flux_es(is) + T_s(is) * ( 1._DP + eta_s(is) )  &
                                              * 2._DP * p_flux_es(mx,my,is)
        end do
      end do
      mx   = 0
        do my = 1, ny
      ...
      do iz = -global_nz, global_nz-1
        do my = -ny, ny
          do mx = 0, nx
            wr3(mx,my,iz) = real( &
              conjg( ui * ky(my) * gAl(mx,my,iz) ) * gupara(mx,my,iz,is), kind=DP )
          end do
        end do
      end do

      call thet_ave_r ( wr3, p_flux_em(:,:,is) )

      do my = -ny, ny
        do mx = 1, nx
          tp_flux_em(is) = tp_flux_em(is) + T_s(is) * ( 1._DP + eta_s(is) )  &
                                              * 2._DP * p_flux_em(mx,my,is)
        end do
      end do
      mx   = 0
        do my = 1, ny
      ...
      do iz = -global_nz, global_nz-1
        do my = -ny, ny
          do mx = 0, nx
            wr3(mx,my,iz) = real( &
              conjg( - ui * ky(my) * gphi(mx,my,iz) ) &
                       * ( gppara(mx,my,iz,is) + gpperp(mx,my,iz,is) &
                             - 2.5_DP * T_s(is) * gdens(mx,my,iz,is) ), kind=DP )
          end do
        end do
      end do
      call thet_ave_r ( wr3, h_flux_es(:,:,is) )

      do my = -ny, ny
        do mx = 1, nx
          th_flux_es(is) = th_flux_es(is) + eta_s(is)  &
                                              * 2._DP * h_flux_es(mx,my,is)
        end do
      end do
      mx   = 0
        do my = 1, ny
      ...
      do iz = -global_nz, global_nz-1
        do my = -ny, ny
          do mx = 0, nx
            wr3(mx,my,iz) = real( &
              conjg( ui * ky(my) * gAl(mx,my,iz) ) &
                       * ( gqlpara(mx,my,iz,is) + gqlperp(mx,my,iz,is) &
                              - 2.5_DP * T_s(is) * gupara(mx,my,iz,is) ), kind=DP )
          end do
        end do
      end do

      call thet_ave_r ( wr3, h_flux_em(:,:,is) )

      do my = -ny, ny
        do mx = 1, nx
          th_flux_em(is) = th_flux_em(is) + eta_s(is)  &
                                              * 2._DP * h_flux_em(mx,my,is)
        end do
      end do
      mx   = 0
        do my = 1, ny
    ...
    write( otfl, "(99G17.7E3)" ) time, tp_flux_es(1:ns), tp_flux_em(1:ns),  &
...
!%%% Spectra of fluxes %%%
    ...
    write( pflk, * ) "splot './plt/fluxinkxky_t"//cloop//"."//cnum//"' u 1:2:3"
    write( pflk, * ) "pause 0.2"

    open( oflk, file="./plt/fluxinkxky_t"//cloop//"."//cnum )
    write( oflk, * ) "#  time = ", time 
    write( oflk, "(99a17)" ) "#              kx","ky",            &
                             "p_flux_es(1:ns)","p_flux_em(1:ns)", &
                             "h_flux_es(1:ns)","h_flux_em(1:ns)"
    do my = -ny ,ny
      do mx = 0, nx
        write( oflk, "(99G17.7E3)" ) kx(mx), ky(my),                                &
                                     p_flux_es(mx,my,1:ns), p_flux_em(mx,my,1:ns),  &
                                     h_flux_es(mx,my,1:ns), h_flux_em(mx,my,1:ns)
      end do
      write( oflk, * )
    end do
    close( oflk )
