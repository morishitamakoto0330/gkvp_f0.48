...
! --- local variables
  ...
  real(kind=DP) :: time
  complex(kind=DP),  &
    dimension(0:nx,-ny:ny,-nz:nz-1)               :: Al
  complex(kind=DP),  &
    dimension(0:nx,-ny:ny,-global_nz:global_nz-1) :: gAl
  integer :: mx, my, iz, giz

    write( cnum, fmt="(i3.3)" ) inum
    write( olog, * ) " # Analysis Al."

! --- file open ---
    do ir = 0, nprocz - 1
      write( crank, fmt="(i4.4)" ) ir
      open( unit=1000+ir, file="./bdata/gkv."//crank//".Al."//cnum,  &
            status="old", action="read", form="unformatted" )
      write( olog, * ) " # Opened file and the unit-ID are ",  &
                       "./bdata/gkv."//crank//".Al."//cnum, 1000+ir
    end do
...
! --- time-step loop ---
  ...
  if ( loop >= 0 ) then            !%%% to restart
      ...
      do ir = 0, nprocz - 1
        ...
        read( unit=1000+ir, iostat=ios ) time, Al
        if ( ios /= 0 ) exit
