...
!%%% Set parameters %%%
  ...
  integer, parameter :: nxw = 9, nyw = 3
  integer, parameter :: nx = 6, global_ny = 1  ! 2/3 de-aliasing rule
  integer, parameter :: global_nz = 16, global_nv = 32, global_nm = 15
  integer, parameter :: nzb = 2
  integer, parameter :: nprocw = 2, nprocz = 2, nprocv = 8, nprocm = 2, nprocs = 2
...
!     Initial setting
    ...
    namelist /confp/ r_minor, r_major, eps_r, eps_rnew,     &
      ...
      open( 5, file="./gkvp_f0.27_namelist."//cnum, status="old", action="read" )
  ...
  ! --- file open ---
      do ir = 0, nproc - 1
        ...
        if ( ranks == 0 .AND. vel_rank == 0 ) then
          ...
          open( unit=100000+ir, file="./phi/gkvp_f0.27."//crank//"."//srank//".phi."//cnum,  &
                status="old", action="read", form="unformatted" )
          write( olog, * ) " # Opened file and the unit-ID are ",  &
                           "./phi/gkvp_f0.27."//crank//"."//srank//".phi."//cnum, 100000+ir
          open( unit=200000+ir, file="./phi/gkvp_f0.27."//crank//"."//srank//".Al."//cnum,  &
                status="old", action="read", form="unformatted" )
          write( olog, * ) " # Opened file and the unit-ID are ",  &
                           "./phi/gkvp_f0.27."//crank//"."//srank//".Al."//cnum, 200000+ir
        ...
        if ( vel_rank == 0 ) then
          ...
          write( crank, fmt="(i6.6)" ) ir
          write( srank, fmt="(i1.1)" ) ranks
          open( unit=300000+ir, file="./phi/gkvp_f0.27."//crank//"."//srank//".mom."//cnum,  &
                status="old", action="read", form="unformatted" )
          write( olog, * ) " # Opened file and the unit-ID are ",  &
                           "./phi/gkvp_f0.27."//crank//"."//srank//".mom."//cnum, 300000+ir
