...
!%%% Set parameters %%%
  ...
  integer, parameter :: nxw = 144, nyw = 36
  integer, parameter :: nx = 96, global_ny = 24  ! 2/3 de-aliasing rule
  integer, parameter :: global_nz = 32, global_nv = 40, global_nm = 15
  integer, parameter :: nzb = 2
  integer, parameter :: nprocw = 4, nprocz = 8, nprocv = 10, nprocm = 2, nprocs = 2
  real(kind=DP), parameter ::  vmax = 5._DP
!%%%%%%%%%%%%%%%%%%%%%%
...
!     Initial setting
    ...
    namelist /confp/ r_minor, r_major, eps_r, eps_rnew,     &
      ...
      open( 5, file="./gkvp_f0.28_namelist."//cnum, status="old", action="read" )
...
!     Data analysis from *.cnt.*
  ...
  ! --- file open ---
      do ir = 0, nproc - 1
        ...
        open( unit=100000+ir, file="./cnt/gkvp_f0.28."//crank//"."//srank//".cnt."//cnum,  &
              status="old", action="read", form="unformatted" )
        write( olog, * ) " # Opened file and the unit-ID are ",  &
                         "./cnt/gkvp_f0.28."//crank//"."//srank//".cnt."//cnum, 100000+ir
...
!     Data analysis from *.phi.*, *.Al.*, *.mom.*
  ...
  ! --- file open ---
      do ir = 0, nproc - 1
        ...
        if ( ranks == 0 .AND. vel_rank == 0 ) then
          ...
          write( crank, fmt="(i6.6)" ) ir
          write( srank, fmt="(i1.1)" ) ranks
          open( unit=100000+ir, file="./phi/gkvp_f0.28."//crank//"."//srank//".phi."//cnum,  &
                status="old", action="read", form="unformatted" )
          write( olog, * ) " # Opened file and the unit-ID are ",  &
                           "./phi/gkvp_f0.28."//crank//"."//srank//".phi."//cnum, 100000+ir
          open( unit=200000+ir, file="./phi/gkvp_f0.28."//crank//"."//srank//".Al."//cnum,  &
                status="old", action="read", form="unformatted" )
          write( olog, * ) " # Opened file and the unit-ID are ",  &
                           "./phi/gkvp_f0.28."//crank//"."//srank//".Al."//cnum, 200000+ir
        ...
        if ( vel_rank == 0 ) then
          ...
          write( crank, fmt="(i6.6)" ) ir
          write( srank, fmt="(i1.1)" ) ranks
          open( unit=300000+ir, file="./phi/gkvp_f0.28."//crank//"."//srank//".mom."//cnum,  &
                status="old", action="read", form="unformatted" )
          write( olog, * ) " # Opened file and the unit-ID are ",  &
                           "./phi/gkvp_f0.28."//crank//"."//srank//".mom."//cnum, 300000+ir
  ...
  ! --- time-step loop ---
    ...
    if ( loop >= 0 ) then            !%%% to restart
        ...
        if ( ios < 0 ) then
          write( olog, * ) " # All files are completed. iostat, unit-ID = ",  &
                           ios, 100000+ir, 200000+ir, 300000+ir
          write( olog, * ) ""
          exit
        else if ( ios > 0 ) then
          write( olog, * ) " # I/O error is detected. iostat, unit-ID = ",  &
