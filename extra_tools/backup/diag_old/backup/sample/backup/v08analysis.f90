...
!%%% Set parameters %%%
  ...
  integer, parameter :: nxw = 3, nyw = 3
  integer, parameter :: nx = 0, global_ny = 1  ! 2/3 de-aliasing rule
  integer, parameter :: global_nz = 736, global_nv = 32, global_nm = 15
  integer, parameter :: nzb = 2
  integer, parameter :: nprocw = 1, nprocz = 16, nprocv = 4, nprocm = 2, nprocs = 2
...
!  Execution of FFT
  ...
  !  complex(kind=DP), dimension(0:2*nxw-1,0:nyw) :: wwkk
    ...
    !  call dfftw_execute_dft( plan_backward_xy, wwkk, wwxy )
...
!     Open files
    ...
    implicit none
      ...
      open( oxti, file="./plt/tfluxes.dat" )
        write( oxti, "(99a17)" ) "#            Time",               &
                                 "p_flux_es(ns)", "p_flux_em(ns)",  &
                                 "h_flux_es(ns)", "h_flux_em(ns)"
  
      open( pxti, file="./plot_tfluxes.gn" )
        write( pxti, * ) "set xlabel 'Time t v_{ti}/L_n'"
        write( pxti, * ) "set ylabel 'Particle Fluxes'"
        write( pxti, * ) "plot './plt/tfluxes.dat' u 1:2 ti '{/Symbol G}_{eE}' w l, \\"
        write( pxti, * ) "                      '' u 1:3 ti '{/Symbol G}_{iE}' w l, \\"
        write( pxti, * ) "                      '' u 1:4 ti '{/Symbol G}_{eM}' w l, \\"
        write( pxti, * ) "                      '' u 1:5 ti '{/Symbol G}_{iM}' w l"
        write( pxti, * ) "pause -1"
        write( pxti, * ) "set ylabel 'Heat Fluxes'"
        write( pxti, * ) "plot './plt/tfluxes.dat' u 1:6 ti 'Q_{eE}' w l, \\"
        write( pxti, * ) "                      '' u 1:7 ti 'Q_{iE}' w l, \\"
        write( pxti, * ) "                      '' u 1:8 ti 'Q_{eM}' w l, \\"
        write( pxti, * ) "                      '' u 1:9 ti 'Q_{iM}' w l"
        write( pxti, * ) "pause -1"
      ...
      open( oxtk, file="./plt/fluxintky.dat" )
        write( oxtk, "(99a17)" ) "#            Time", "ky",         &
                                 "p_flux_es(ns)", "p_flux_em(ns)",  &
                                 "h_flux_es(ns)", "h_flux_em(ns)",  &
                                 "p_ampl_es(ns)", "p_ampl_em(ns)",  &
                                 "h_ampl_es(ns)", "h_ampl_em(ns)"
  
      open( pxtk, file="./plot_fluxintky.gn" )
        write( pxtk, * ) "set pm3d map"
        write( pxtk, * ) "set size square"
        write( pxtk, * ) "set xlabel 'Time t'"
        write( pxtk, * ) "set ylabel 'Wave number ky'"
        write( pxtk, * ) "#set palette define ( -1 'blue', 0 'white', 1 'red' )"
        write( pxtk, * ) "splot './plt/fluxintky.dat' u 1:2:3"
        write( pxtk, * ) "pause -1"
...
!     Data analysis from *.phi.*, *.Al.*, *.mom.*
  ...
  ! --- time-step loop ---
    ...
    if ( loop >= 0 ) then            !%%% to restart
        ...
        call fluxintky( inum, loop, time, gphi, gAl,  &
                        gdens, gupara, gppara, gpperp, gqlpara, gqlperp )
...
!     Write phi in x, y space
    ...
    use analysis_header
    use analysis_fft, only : fft_backward_xy
...
!     Write particle and heat fluxes in kx, ky
  ...
  !--- Magnetic part of heat fluxes ---
      ...
      write( oxti, "(99G17.7E3)" ) time, tp_flux_es(0:ns-1), tp_flux_em(0:ns-1),  &
...
!--------------------------------------
  SUBROUTINE fluxintky( inum, loop, time, gphi, gAl,  &
                        gdens, gupara, gppara, gpperp, gqlpara, gqlperp )
!--------------------------------------
!     Write particle and heat fluxes in time, ky

    use analysis_header
    use analysis_fft, only : fft_backward_x
  ...
  !--- Magnetic part of heat fluxes ---
      ...
      do my = 0, global_ny
        write( oxtk, "(99G17.7E3)" ) time, gky(my), p_flux_es(my,0:ns-1),  &
                                                    p_flux_em(my,0:ns-1),  &
                                                    h_flux_es(my,0:ns-1),  &
                                                    h_flux_em(my,0:ns-1),  &
                                                    p_ampl_es(my,0:ns-1),  &
