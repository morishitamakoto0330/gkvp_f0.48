...
!%%% DIAG parameters %%%
  ...
  logical, parameter :: flag_mominxy   = .false.
  logical, parameter :: flag_mominz    = .false.
  logical, parameter :: flag_mominxyz  = .true.
  logical, parameter :: flag_trn = .false.
  logical, parameter :: flag_trninkxky = .false.
!%%%%%%%%%%%%%%%%%%%%%%
...
!$OMP parallel do
      ...
      open( omominkxky, file="./plt/mominkxky_t"//cloop//".dat" )
        write( omominkxky, "(a17i17a17G17.7E3)" )  &
                                       "#           loop=",loop, "time=",time
        write( omominkxky, "(99a17)" ) "#              kx","ky","||phi||^2","||Al||^2",       &
                                       "||dens||^2","||upara||^2","||pres||^2","||qpara||^2", &
...
!     Write 3D data of phi in vtk format
  ...
  ! --- local variables
    ...
    real(kind=DP), dimension(0:2*nxw,0:2*nyw,-global_nz:global_nz) :: wr3
        ...
        if ( loop >= 0 .and. mod(loop,loopskip)==0 ) then  !%%% loop control
          ...
          do ir = 0, nproc - 1
            ...
            !  read( unit=500000+ir, iostat=ios ) time, entrpy, neint, nmint, dcd
              read( unit=500000+ir, iostat=ios, rec=row ) head, time, entrpy, neint, nmint, dcd, foot
              if ( ios /= 0 ) exit
